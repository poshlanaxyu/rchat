...
function Network.connect()
    ...
    if res then
        ...
        sampAddChatMessage("RdugChat: ����������!", 0x00FF00) -- ��������
...
function Network.disconnect()
    if State.connected then sampAddChatMessage("RdugChat: ������ ����������...", 0xFF0000) end -- ��������
...
PacketHandlers['online'] = function(msg)
    -- ��������
    sampAddChatMessage("����� ����������� ���� ������, ����� {D8A903}" .. #msg.clients .. "{FFFFFF} �������:", 0xFFFFFF)
    for _, v in ipairs(msg.clients) do
        ...
        sampAddChatMessage(string.format("���: {abcdef}%s - %s {ffffff}����:{fbec5d} %s%s%s", v.nick, v.id, u8:decode(v.rank), afk, wlow), 0xFFFFFF)
    end
end
...
function se.onServerMessage(color, message)
	if message:find("{FF4500} ����� ���� ����������.") or message:find("{FF4500} ������ ���� ����������.") then
		State.is_z = true
		Network.send("chat", { text = u8(message), nick = Utils.getPlayerNick(), id = Utils.getPlayerId() })
	end

	if message:find("{FF4500} ���������� ���� ����������.") or message:find("{FF4500} ��������� ���� ����������.") or message:find("��� �� ������ ��������� ���������� ��. � ��������� {33aa33}") then
		State.is_z = false
		Network.send("chat", { text = u8(message), nick = Utils.getPlayerNick(), id = Utils.getPlayerId() })
	end

    if State.send_wlow and message:find("�� �� ���������� � �������.") then
...
function se.onShowTextDraw(id, data)
    ...
    -- ������������ ������ ��� �������
    if data.text:find("HA �AC HA�A� ��POK ~r~") then

        local attacker_name = data.text:gsub(".*HA �AC HA�A� ��POK ~r~",""):gsub("~w~.~n~.*","")
        for id, name in pairs(Utils.getAllSampPlayers()) do
            if string.lower(name):find(string.lower(attacker_name)) then
                if not State.attackers[id] then
                    ...
                    Network.send("chat", { text = u8("{FF6666}!!! ������� �� {"..Utils.hex_color(color).."}"..name.." ["..id.."]"), nick = Utils.getPlayerNick(), id = Utils.getPlayerId() })
                end
            end
        end
    end
...
function se.onShowDialog(id, style, title, btn1, btn2, text)
	if State.send_wlow and title:find("{34C924}���������� � ����� �������") then
        local stars_us = 0
        local stars_af = 0
        local stars_rc = 0
        local stars_int = 0

        for country, count in text:gmatch("������ �� (%u+) %- (%d+)") do
...
function main()
    ...
    sampAddChatMessage("RdugChat: /u [�����].", 0xAAAAAA) -- ��������
    
    sampRegisterChatCommand("u", function(arg)
        if #arg > 0 then
            Network.send("chat", { text = u8(arg), nick = Utils.getPlayerNick(), id = Utils.getPlayerId() }) else sampAddChatMessage("�����������: /u [�����]", -1)
        end
    end)
    sampRegisterChatCommand("ugps", function() 
        ...
        -- ��������
        sampAddChatMessage(State.gps_enabled and "[����] {FFFFFF}GPS �������!" or "[����] {FFFFFF}GPS ��������!", 0xfbec5d) 
    end)
