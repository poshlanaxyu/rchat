...
function Network.connect()
    ...
    if res then
        ...
        sampAddChatMessage("RdugChat: Ïîäêëş÷åíî!", 0x00FF00) -- ÎĞÈÃÈÍÀË
        Network.send("login", {
...
function Network.disconnect()
    if State.connected then sampAddChatMessage("RdugChat: Ïîòåğÿ ñîåäèíåíèÿ...", 0xFF0000) end -- ÎĞÈÃÈÍÀË
...
PacketHandlers['online'] = function(msg)
    -- ÎĞÈÃÈÍÀË
    sampAddChatMessage("×ëåíû ïîäâàëüíîãî ÷àòà îíëàéí, âñåãî {D8A903}" .. #msg.clients .. "{FFFFFF} ÷åëîâåê:", 0xFFFFFF)
    for _, v in ipairs(msg.clients) do
        local afk = ""
        if sampIsPlayerPaused(v.id) then afk = " {34C924}< AFK >" end
        sampAddChatMessage(string.format("Íèê: {abcdef}%s - %s {ffffff}Ğàíã:{fbec5d} %s%s", v.nick, v.id, u8:decode(v.rank), afk), 0xFFFFFF)
    end
end
...
function se.onShowTextDraw(id, data)
    -- ÎĞÈÃÈÍÀËÜÍÀß ÑÒĞÎÊÀ ÄËß ÒĞÈÍÈÒÈ
    if data.text:find("HA ‹AC HAŒA‡ …‚POK ~r~") then

        local attacker_name = data.text:gsub(".*HA ‹AC HAŒA‡ …‚POK ~r~",""):gsub("~w~.~n~.*","")
        for id, name in pairs(Utils.getAllSampPlayers()) do
            if string.lower(name):find(string.lower(attacker_name)) then
                if not State.attackers[id] then
                    ...
                    Network.send("chat", { text = u8("{FF6666}!!! Îáîğîíà ïî {"..Utils.hex_color(color).."}"..name.." ["..id.."]"), nick = Utils.getPlayerNick(), id = Utils.getPlayerId() })
                end
            end
        end
    end
...
function main()
    ...
    sampAddChatMessage("RdugChat: /u [òåêñò].", 0xAAAAAA) -- ÎĞÈÃÈÍÀË
    
    sampRegisterChatCommand("u", function(arg)
        if #arg > 0 then
            Network.send("chat", { text = u8(arg), nick = Utils.getPlayerNick(), id = Utils.getPlayerId() }) else sampAddChatMessage("Èñïîëüçóéòå: /u [òåêñò]", -1)
        end
    end)
